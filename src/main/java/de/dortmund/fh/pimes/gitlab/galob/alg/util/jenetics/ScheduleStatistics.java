 ...
 *
 */
public class ScheduleStatistics extends JPanel
    implements TreeSelectionListener, Consumer<EvolutionResult<ScheduleGene, Double>> {
  /**
   * Default Serial ID.
   */
  ...
  private void createGeneralItems(DefaultMutableTreeNode top) {
    ...
    chart.addSeries("Fitness", generations, this.bestFitnessHistory).setMarker(SeriesMarkers.NONE);

    DefaultMutableTreeNode defaultMutableTreeNode =
        new DefaultMutableTreeNode(new PaneInfo("Best fitness Evolution", chart));
    ...
    // Calculate the mean
    final double mean =
        this.actualFitnessHistory.stream().mapToDouble(i -> i).average().getAsDouble();

    // Calculate the variance
    final double var =
        this.actualFitnessHistory.stream().mapToDouble(i -> Math.pow((i - mean), 2)).sum()
    ...
    // Time distribution
    PieChart pieChart = new PieChartBuilder().title("Time Distribution").build();
  ...
  private void createMakespanItems(DefaultMutableTreeNode top) {
    ...
    DefaultMutableTreeNode defaultMutableTreeNode =
        new DefaultMutableTreeNode(new PaneInfo("Best schedule evolution", chart));
    category.add(defaultMutableTreeNode);
    ...
    defaultMutableTreeNode =
        new DefaultMutableTreeNode(new PaneInfo("Per Computing Resources (Bars)", catChart));
    category.add(defaultMutableTreeNode);
  }
  ...
  private void createSimulatedAnnealingItems(DefaultMutableTreeNode top) {
    ...
    chart.addSeries("Temperature(n)", generations, saTemp).setMarker(SeriesMarkers.NONE);

    DefaultMutableTreeNode defaultMutableTreeNode =
        new DefaultMutableTreeNode(new PaneInfo("Temperature Evolution", chart));
    category.add(defaultMutableTreeNode);
  ...
  public void showStats() {
    ...
    Chart<?, ?> firstChart =
        (Chart<?, ?>) ((PaneInfo) ((DefaultMutableTreeNode) top.getChildAt(0).getChildAt(0))
            .getUserObject()).getCookie();
